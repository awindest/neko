<script lang="ts">
	import { mousePos } from '$lib/runes/runes.svelte.js'
	...
	const nekoSpeed = 20 // 10
	const nekoOffset = 0 // 16
	...
	function frame() {
		frameCount += 1
		const diffX = nekoPosX - mousePos.X
		const diffY = nekoPosY - mousePos.Y
		...
		nekoPosX = Math.min(Math.max(16, nekoPosX), window.innerWidth - nekoOffset)
		nekoPosY = Math.min(Math.max(16, nekoPosY), window.innerHeight - nekoOffset)
	}
